...
export default function InvoicePreviewModal({ isOpen, onClose, invoiceData }: InvoicePreviewModalProps) {
  ...
  const subtotal = invoiceData.items?.reduce((sum: number, item: any) => sum + item.quantity * item.price, 0) || 0
  const vatAmount = invoiceData.vatAmount || 0
  const total = subtotal + vatAmount
  const payments = invoiceData.payments || 0
    ...
    >
      ...
      >
        ...
        <div className="no-print sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center z-10">
          <h3 className="text-xl font-semibold text-slate-800">Invoice Preview</h3>
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-slate-50 print:p-0 print:overflow-visible print:bg-white">
          <div className="print-content bg-white p-4 rounded-lg shadow-sm print:shadow-none print:rounded-none">
            ...
            <div className="flex justify-between items-start mb-4 pb-3 border-b border-slate-200">
              ...
              <div className="text-right">
                ...
                <div className="text-sm text-slate-600">
                  <div>nº {invoiceData.invoiceNumber || 'INV00001'}</div>
                  <div className="mt-1">
                    <div>Invoice Date: {formatDate(invoiceData.date) || '11 Jan 2026'}</div>
                    <div>Due date: {formatDate(invoiceData.dueDate) || formatDate(invoiceData.date) || '25 Jan 2026'}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mb-4 pb-3 border-b border-slate-200">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-slate-600">
                    <p className="font-medium">{invoiceData.selectedCustomer || 'Customer Name'}</p>
                    {invoiceData.customerAddress && (
                      <p className="mt-0.5">{invoiceData.customerAddress}</p>
                    )}
                    {invoiceData.customerEmail && (
                      <p className="mt-0.5">{invoiceData.customerEmail}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="w-80">
                <table className="w-full">
                  <tbody>
                    ...
                    <tr>
                      <td className="py-1.5 px-3 text-right text-sm text-slate-700">Invoice Total</td>
                      <td className="py-1.5 px-3 text-right text-sm font-semibold text-slate-800">{total.toFixed(2)}</td>
                    </tr>
                    {payments > 0 && (
                      <tr>
                        <td className="py-1.5 px-3 text-right text-sm text-slate-700">Less Payments (GBP)</td>
                        <td className="py-1.5 px-3 text-right text-sm font-semibold text-slate-800">{payments.toFixed(2)}</td>
                      </tr>
                    )}
                    <tr className="bg-blue-50">
                      <td className="py-1.5 px-3 text-right text-sm font-semibold text-blue-700">Balance to pay (GBP)</td>
                      <td className="py-1.5 px-3 text-right text-sm font-bold text-blue-700">{balance.toFixed(2)}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-3 pt-3 border-t border-slate-200">
              <div>
                <div className="text-xs text-slate-600 mb-1">Payment method - GBP</div>
              </div>
              <div className="text-xs text-slate-600">
                {invoiceData.terms && (
                  <div className="mb-1 leading-tight">{invoiceData.terms}</div>
                )}
                {invoiceData.dueDate && (
                  <div className="leading-tight">Payment is due by {formatDate(invoiceData.dueDate)}.</div>
                )}
                {!invoiceData.dueDate && invoiceData.timeToPay && (
                  <div className="leading-tight">Payment is due {invoiceData.timeToPay} days from invoice date.</div>
                )}
                {!invoiceData.dueDate && !invoiceData.timeToPay && (
                  <div className="leading-tight">Payment is due 14 days from invoice date.</div>
                )}
              </div>
            </div>
